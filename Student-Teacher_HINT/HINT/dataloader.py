...
def get_sentence2vec():
    from HINT.protocol_encode import load_sentence_2_vec, save_sentence_bert_dict_pkl
    file_path = os.path.join("..", "data", "sentence2embedding.pkl")
    if not os.path.exists(file_path):
        print("Embedding file not found. Generating new embeddings...")
        save_sentence_bert_dict_pkl()
    return load_sentence_2_vec()
...
def smiles_txt_to_2lst(smiles_txt_file):
	with open(smiles_txt_file, 'r', encoding="utf-8") as fin:
		lines = fin.readlines() 
...
def generate_admet_dataloader_lst(batch_size):
	datafolder = os.path.join("..", "data/ADMET/cooked/")
