...
class HINTClassroom:
    ...
    def validate_student(self, valid_loader):
        ...
        with torch.no_grad():
            for batch in valid_loader:
                ...
                loss_val = self.student.loss(output, label_vec)
    ...
    def validate_teacher(self, valid_loader):
        ...
        with torch.no_grad():
            for batch in valid_loader:
                ...
                loss_val = self.teacher.loss(output, label_vec)
                total_loss += loss_val.item()
                count += 1
        self.teacher.train()
        return total_loss / count if count > 0 else float("inf")
    ...
    def learn(self, train_loader, valid_loader, test_loader, base_name, path, num_epochs=10, lr=1e-3, weight_decay=0):
        """
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader):
                ...
                # Student
                student_out, student_emb = self.student.forward(
                    smiles_lst2, icdcode_lst3, criteria_lst, return_embeddings=True
                )
                student_out = student_out.view(-1)
                loss_student = naive_loss_fn(student_out, label_vec)

                # Teacher
                teacher_out, teacher_emb = self.teacher.forward(
                    smiles_lst2, icdcode_lst3, criteria_lst, return_embeddings=True
                )
                teacher_out = teacher_out.view(-1)
                loss_teacher_BCE = naive_loss_fn(teacher_out, label_vec)

                # Gradients
                grad_student = torch.autograd.grad(
                    outputs=student_out,
                    inputs=student_emb,
                    grad_outputs=torch.ones_like(student_out),
                    create_graph=True,
                    allow_unused=True
                )[0]
                if grad_student is None:
                    print("We have no gradient, kiddo!")
                    grad_student = torch.zeros_like(student_emb)

                grad_teacher = torch.autograd.grad(
                    outputs=teacher_out,
                    inputs=teacher_emb,
                    grad_outputs=torch.ones_like(teacher_out),
                    create_graph=True,
                    allow_unused=True
                )[0]
                if grad_teacher is None:
                    print("We have no gradient, teach!")
                    grad_teacher = torch.zeros_like(teacher_emb)

                # Example: "orthogonal exploration"
                ...
                print(f'loss_teacher_BCE: {loss_teacher_BCE}')
                print(f'cosine_similarity: {loss_grad ** 0.25}')
                print(f'cosine_penalty: {loss_grad}')
            ...
            student_ckpt = os.path.join(path, f"cos_{base_name}_{self._lambda}_{self.d}_student_{epoch+1}.ckpt")
            teacher_ckpt = os.path.join(path, f"cos_{base_name}_{self._lambda}_{self.d}_teacher_{epoch+1}.ckpt")
            torch.save(self.student, student_ckpt)
            torch.save(self.teacher, teacher_ckpt)

            stud_val_loss, teach_val_loss = self.validate(valid_loader)

            self._lambda += self.d

            print(f"Epoch {epoch+1}: Student Loss={loss_student.item():.4f}, Teacher Loss={loss_teacher.item():.4f}")
            print(f"            : Teacher BCE={loss_teacher_BCE.item():.4f}, CosPenalty={loss_grad.item():.4f}")
            print(f"            : StudentVal={stud_val_loss:.4f}, TeacherVal={teach_val_loss:.4f}")

        # Could do final test, but you said minimal changes, so we leave it
