...
class HINTClassroom:
	...
	def learn(self, train_loader, valid_loader, test_loader, path, base_name, num_epochs=10, lr=1e-3, weight_decay=0):
		...
		for epoch in range(num_epochs):
			...
			for batch in tqdm(train_loader):
				...
				loss_grad = mse_loss_fn(norm_grad_teacher, -norm_grad_student)
			...
			student_ckpt_path = os.path.join(path, f"{base_name}_{self._lambda}_{self.d}_student_{epoch+1}.ckpt")
			teacher_ckpt_path = os.path.join(path, f"{base_name}_{self._lambda}_{self.d}_teacher_{epoch+1}.ckpt")

			print(f"Saving student checkpoint to: {student_ckpt_path}")
			print(f"Saving teacher checkpoint to: {teacher_ckpt_path}")

			torch.save(self.student, student_ckpt_path)
			torch.save(self.teacher, teacher_ckpt_path)
